...
namespace RigidChips {
	...
	///トリムフレーム チップ クラス
	///</summery>
	public class TrimFrameChip	: TrimChip{
		bool option;
		XFile mesh2;
		public TrimFrameChip(){}
		public TrimFrameChip(Environment gen,ChipBase parent,JointPosition pos) : base(gen,parent,pos){
			mesh2 = Environment.GetMesh("TrimF.x");
				...
				if(Environment.DrawOption.FrameGhostShow){
					switch(Environment.DrawOption.FrameGhostView){
						...
						case 1:
							...
							break;
						case 2:
							XFile x = Environment.GetMesh(Application.StartupPath + "\\Resources\\TrimG.x",true);
		...
		public override ChipBase Clone(bool WithChild,ChipBase parent) {
			TrimFrameChip copy = new TrimFrameChip();
			copy.Name = this.Name;
