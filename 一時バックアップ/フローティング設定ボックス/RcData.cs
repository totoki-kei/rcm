...
namespace RigidChips {
	...
	public class RcChipBase{
		...
		public RcChipBase(RcData gen,RcChipBase parent,RcJointPosition pos){
			...
			if(parent != null)
				...
				this.ChipColor = RcColor.Default;
		}
		...
		public override string ToString(){
			string str = "";
			string comma = Generics.OutputOption.CommaWithSpace ? ", " : ",";
			str += this.ChipType + "(";

			string[] attrname = this.AttrNameList;
			float[] attrdef = this.AttrDefaultValueList;
			RcAttrValue[] attrval = this.AttrValList;
			if(this.Name != null && this.Name != "")str += "Name=" + Name + comma;
			if(this.ChipColor != RcColor.Default)
				str += "Color=" + ChipColor.ToString() + comma;

			for(int i = 0;i < attrname.Length;i++){
				if(Generics.OutputOption.PrintAllAttributes || attrval[i].Val != null || attrval[i].Const != attrdef[i])
					str += attrname[i] + "=" + attrval[i].ToString() + comma;
			}
		...
		public virtual string ToString(int tabs) {
			string s = "",st = "",ss = "";
			if(Generics.OutputOption.IndentEnable)for(int i = 0;i < tabs;i++)st += (Generics.OutputOption.IndentBySpace ? "".PadLeft((int)Generics.OutputOption.IndentNum,' ') : "\t");
			s += st;
			//	接続方位
			...
			//	派生ブロック取得
			if(Generics.OutputOption.OpenBracketWithChipDefinition){

				if(!Generics.OutputOption.ReturnEndChipBracket){
					foreach(RcChipBase cb in this.Child)if(cb != null)
															ss += "\n" + cb.ToString(tabs + 1);
					if(ss == "")
						s += "{}" + ((this.Comment == null || this.Comment == "") ? "" : ("//" + this.Comment));
					else
						s += "{" + ((this.Comment == null || this.Comment == "") ? "" : ("//" + this.Comment)) + ss + "\n" + st + "}";
				}
				else{
					s += "{" + ((this.Comment == null || this.Comment == "") ? "" : ("//" + this.Comment));
					foreach(RcChipBase cb in this.Child)if(cb != null)
				...
				foreach(RcChipBase cb in this.Child)if(cb != null)
														s += "\n" + cb.ToString(tabs + 1);
				if(!Generics.OutputOption.ReturnEndChipBracket && s.EndsWith("{"))
					s += "}";
				else
					s += "\n" + st + "}";
		...
		/// <returns>同一の内容を保持したRcChipBaseへの参照</returns>
			...
			try{
				for( ; param[index] != "" ; index += 2){
					...
					if(param[index] == "name")
						...
						newchip.ChipColor = RcColor.Parse(param[index+1]);
					else{
	...
	public class RcData{
		...
		public int SelectedChipCount{
			get{
				return (multiSelect ? selectedCount : 1);
			}
			set{
				if(value != 0)
		...
		public RcChipBase[] SelectedChipList{
			get{
				if(multiSelect)
				return selectedChipArray.ToArray(model.root.GetType());
				else return null;
			}
		}
		/// <summary>
		...
		public void DrawCursor(Color CursorColor){
			Cursor.ChipColor.Read(CursorColor.ToArgb());
			Cursor.DrawChipAll();
		}
		...
		public void CheckBackTrack(){
			...
			for(int i = 0;i < MaxChipCount -1; i++){
				...
				if(ChipLib[i].Parent.RegistID < 0){
					try{UnregisterChipAll(ChipLib[i].Parent);}catch{}
					continue;
				}
		...
		public RcChipBase SelectedChip{
			...
			set{
				SetCursor(value);
				if(!selectedChipArray.Contains(value)){
					multiSelect = false;
					selectedChipArray.Clear();
					SelectedChipCount = 0;
				}
				if(OnSelectedChipChanged != null)OnSelectedChipChanged(value);
			}
		}
		...
		public void AssignSelectedChips(RcChipBase chip){
			...
			selectedChipCount = selectedChipArray.Count;
		}
		...
		public void Save(string FileName){
			...
			foreach(RcVal v in vals.list){
				file.WriteLine("{0},{1},{2},{3},{4},{5}",v.ValName.ToString(formatinfo),v.Default.ToString(formatinfo),v.Min.ToString(formatinfo),v.Max.ToString(formatinfo),v.Step.ToString(formatinfo),v.Disp.ToString(formatinfo));
			}
			...
			for(int i = 0;i < MaxChipCount;i++){
				...
				buffer += target.Name + ",";
				buffer += target.ChipColor.ToString() + ",";
		...
		public void Load(string FileName){
			...
			switch(version){
				case "RCMFORMAT VERSION 1.0":
					...
					while(input != null && input != "BODY END"){
						...
						if(parameters[0] == "0"){
							...
							model.root.ChipColor.Read(parameters[3]);
							...
							chip.ChipColor = RcColor.Parse(parameters[3]);
				...
				case "RCMFORMAT VERSION 1.1":
					...
					while(input != null && input != "OTHERS"){
						input = file.ReadLine();
					}
					input = file.ReadLine();
					while(input != null && input != "OTHERS END"){
						//	No Operations.
						input = file.ReadLine();
					}
		...
		public void CalcWeightCenter(){
			...
			totalVector.Scale(1f / totalWeight);
...
/*			set{
	...
	public class RcVal{
		...
		public override string ToString() {
			string buff = "";
			buff += ValName + "(";
			if(Default != 0f)buff += "default=" + Default.ToString("G9") + ",";
			if(Min != 0f)buff += "min=" + Min.ToString("G9") + ",";
			if(!float.IsPositiveInfinity(Max))buff += "max=" + Max.ToString("G9") + ",";
			if(Step != 0)buff += "step=" + Step.ToString("G9") + ",";
			if(!Disp)buff += "disp=0";
			buff = buff.TrimEnd(',');
			buff += ")";
			return buff;
		...
		public float Tick(){
			if(!OnPreview)return this.Default;
			if(Now < Default - Step){
				Now += Step;
			}
	...
	public struct RcAttrValue{
			...
			get{
				...
				if(Val != null){
					if(Val.OnPreview)
						...
						return isNegative ? -Val.Default : Val.Default;
				}
				else	return Const;
			}
		}
		...
		public void SetValue(string expression,RcValList vallist){
			...
			try{
				this.Const = float.Parse(expression);
	...
	public class RcDrawOptions{
		...
		public bool FrameGhostAlpha = true;
		public bool FrameGhostShow = true;
		public bool BaloonSwelling = true;
		public float BaloonSwellingRatio = 1.5f;
		...
		public RcDrawOptions(){
			try{
				...
				FrameGhostAlpha = bool.Parse(file.ReadLine());
		...
		~RcDrawOptions(){
			...
			file.WriteLine(FrameGhostAlpha.ToString());
