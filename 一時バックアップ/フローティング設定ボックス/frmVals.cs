...
namespace rcm
{
	...
	public class frmVals : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
			...
			this.grpPreviewer.Location = new System.Drawing.Point(312, 16);
			...
			// btnUp
			// 
			this.btnUp.Location = new System.Drawing.Point(88, 224);
			...
			// btnDown
			// 
			this.btnDown.Location = new System.Drawing.Point(112, 232);
			...
			// btnApply
			// 
			this.btnApply.Location = new System.Drawing.Point(192, 224);
			...
			// btnCopy
			// 
			this.btnCopy.Location = new System.Drawing.Point(48, 224);
			this.btnCopy.Name = "btnCopy";
			this.btnCopy.Size = new System.Drawing.Size(40, 32);
			this.btnCopy.TabIndex = 20;
			this.btnCopy.Text = "ｺﾋﾟｰ";
		...
		void AdoptValData(){
			...
			try{
				...
				target.Default = float.Parse(txtDefault.Text);
				target.Min = float.Parse(txtMin.Text);
				target.Max = float.Parse(txtMax.Text);
				target.Step = float.Parse(txtStep.Text);

				if(target.Min > target.Max){
					MessageBox.Show("Max / Minの範囲が不正です。\n最大値を最小値に合わせます。","適用エラー",MessageBoxButtons.OK,MessageBoxIcon.Exclamation);
		...
		private void btnAdd_Click(object sender, System.EventArgs e) {
			RcVal NewVal = new RcVal();
			string buff = dlgTextInput.ShowDialog("val" + vallist.Count ,"新しい変数の名前を入力してください",0,@" ""!#$%&'()-=^~|\[]{}`@:*;+,<.>/?",true);
			if(buff != null)
				lstVals.Items.Add(vallist.Add(buff));
		}
		...
		private void lstVals_SelectedIndexChanged(object sender, System.EventArgs e) {
			...
			if(lstVals.SelectedIndex == -1){
				...
				txtDefault.Text = vallist[idx].Default.ToString("G9");
				txtMin.Text = vallist[idx].Min.ToString("G9");
				txtMax.Text = vallist[idx].Max.ToString("G9");
				txtStep.Text = vallist[idx].Step.ToString("G9");
