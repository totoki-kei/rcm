...
//			string s = "";
			...
			if(Generics.OutputOption.IndentEnable){
				if(Generics.OutputOption.IndentBySpace)
					for(int i = 0;i < tabs * Generics.OutputOption.IndentNum ;  i++)st.Append(' ');
				else
			...
			if(Generics.OutputOption.OpenBracketWithChipDefinition){
				...
				if(!Generics.OutputOption.ReturnEndChipBracket && s[s.Length-1] == '{')
					s.Append('}');
				else{
					s.Append("\r\n");
					s.Append(st.ToString());
	...
	public class RcData{
		...
		///選択されているチップのリスト。
		///</summery>
		public object[] SelectedChipList{
			get{
				if(multiSelect)
...
//			throw new Exception("Test finished.");
		...
		internal static bool TryParseNumber(string text) {
			float dummy;
			return TryParseNumber(text, out dummy);
		}
		internal static bool TryParseNumber(string text, out float result) {
			text = text.Trim();
			if (text[0] == '#') {
				int buffer;
...
/*			set{
	...
	public class RcVal{
		...
		public override string ToString() {
			string buff = "";
			buff += ValName + "(";
			if(Default != DefaultDefaultValue)buff += "default=" + Default.ToString("G9") + ",";
			if(Min != DefaultMinValue)buff += "min=" + Min.ToString("G9") + ",";
			if(Max != DefaultMaxValue)buff += "max=" + Max.ToString("G9") + ",";
			if(Step != DefaultStepValue)buff += "step=" + Step.ToString("G9") + ",";
			if(!Disp)buff += "disp=0";
			buff = buff.TrimEnd(',');
			buff += ")";
			return buff;
	...
	public struct RcAttrValue{
		...
		public override string ToString() {
			...
			if(Val != null){
				...
				return "#" + ((int)Const).ToString("X").PadLeft(6,'0');
			else
				return Const.ToString("#########0.########");
		}
		...
		public void SetValue(string expression,RcValList vallist){
			...
			try{
				if(expression[0] == '#'){
					this.Const = int.Parse(expression.Substring(1),System.Globalization.NumberStyles.AllowHexSpecifier) & 0xFFFFFF;
